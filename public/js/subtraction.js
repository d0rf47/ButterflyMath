...
var thisLvlScore = JSON.parse(localStorage.getItem("Lvlscore")||"[]");
var minScore = thisLvlScore;
minScore.hits+=totalHits;
minScore.miss+=totalMiss;
...
let countdown = ()=>{
  ...
  if(count  < 1){
    ...
    let scoreArr =  JSON.parse(localStorage.getItem("Lvlscore"));
    scoreArr.push(score);
    
    localStorage.setItem("Lvlscore", JSON.stringify(scoreArr));
   window.location="../HTML/division.html";
  }
}
